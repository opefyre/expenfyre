...
// Get spending velocity and category performance
app.get('/api/analytics/spending-velocity', authMiddleware, requireAuth, async (c) => {
  try {
  const auth = c.get('auth')
  const analyticsService = getAnalyticsService(c.env)
    ...
    const velocity = await analyticsService.getSpendingVelocity(auth.user.email, filters)
  ...
  return c.json({ 
    ...
    return c.json({ success: false, error: error instanceof Error ? error.message : 'Failed to get spending velocity' }, 500)
  }
})
