...
export default function Layout({ children, user, currentPage, onSignOut }: LayoutProps) {
  ...
  return (
    <div className="h-screen bg-slate-50 overflow-hidden w-full max-w-full">
      {/* Sidebar Overlay */}
      {sidebarOpen && (
        <div 
      ...
      <div className="flex h-full w-full max-w-full">
        ...
        <div className={`flex-1 flex flex-col transition-all duration-300 ease-in-out min-w-0 ${
          ...
          <footer className="bg-white border-t border-slate-200 px-3 sm:px-6 py-4 flex-shrink-0 w-full">
            <div className="flex flex-col sm:flex-row items-center justify-between text-sm text-slate-500 space-y-2 sm:space-y-0 max-w-7xl mx-auto">
